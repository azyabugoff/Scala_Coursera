...
class HuffmanSuite extends munit.FunSuite:
  ...
  trait TestCodeTables {
    val ct1 = List(('a', List[Bit](0, 0)), ('b', List[Bit](0, 1)), ('d', List[Bit](1)))
    val ct2 = convert(Fork(Fork(Leaf('a',2), Leaf('b',3), List('a','b'), 5), Leaf('d',4), List('a','b','d'), 9))
  }

  test("Code bits from code table for \"b\" character") {
    new TestCodeTables:
      assertEquals(codeBits(ct1)('b'), List[Bit](0, 1))
      assertEquals(codeBits(ct2)('b'), List[Bit](0, 1))
  }
