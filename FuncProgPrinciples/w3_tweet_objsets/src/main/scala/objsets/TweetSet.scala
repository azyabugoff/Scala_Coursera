...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet = {
    lazy val leftRec = left.mostRetweeted
    lazy val rightRec = right.mostRetweeted
    if (!left.empty && leftRec.retweets > elem.retweets)
      if (!right.empty && rightRec.retweets > leftRec.retweets)
        rightRec
      else
        leftRec
    if (!right.empty && rightRec.retweets > elem.retweets)
      rightRec
    else
      elem
  }
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets
    .filter(tw => google.exists(x => tw.text.contains(x)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets
    .filter(tw => apple.exists(x => tw.text.contains(x)))
