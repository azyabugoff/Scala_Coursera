...
trait Solver extends GameDef:
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    neighbors.filter{ case (block, lst) => !explored.contains(block) }
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    initial match {
      ...
      case (block, moves) #:: tail => {
        val exploredWithThis: Set[Block] = explored + block
        val nextBlockMoves: LazyList[(Block, List[Move])] = {
          for nxtBlockMvs <- newNeighborsOnly(neighborsWithHistory(block, moves),
                                        exploredWithThis)
          yield nxtBlockMvs
        }
        initial #::: from(tail #::: nextBlockMoves, exploredWithThis)
      }
    }
  }
  ...
  lazy val solution: List[Move] = {
    pathsToGoal match {
      case LazyList() => List()
      case (doneBlock, solutionMoves) #:: tail => solutionMoves.reverse
    }
  }
